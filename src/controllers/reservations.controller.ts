...
import pool from "../db";
import { getOrCreateUser } from "../helpers/users.helper";
...
export async function createReservation(req: Request, res: Response) {
  try {
    ...
    if (!space_id || !start_time || !end_time) {
      res.status(400).json({
        success: false,
        error: "space_id, start_time and end_time are required",
      });
      return;
    }
    ...
    const result = await pool.query(
      ...
      RETURNING *
      `,
      [internalUserId, space_id, start_time, end_time]
    );
...
export async function getAuth0UserReservations(req: Request, res: Response) {
  try {
    const { auth0_id } = req.params;

    const userResult = await pool.query(
      `
      SELECT id, auth0_id, name, email
      FROM users
      WHERE auth0_id = $1
      `,
      [auth0_id]
    ...
    res.status(500).json({
      success: false,
      error: "Failed to fetch Auth0 user reservations",
    });
  }
}